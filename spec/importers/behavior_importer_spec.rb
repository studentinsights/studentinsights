...
RSpec.describe BehaviorImporter do
	...
	let(:import) { behavior_importer.import(file) }
      ...
      it 'does not raise errors about byte sequence, length, missing date/time values' do
      	expect { import }.not_to raise_error
			...
			context 'has exact time' do
				let(:incident_with_time_data) { Student.find_by_state_id("10").reload.discipline_incidents.last }
			...
			context 'time missing' do
				let(:incident_without_time_data) { Student.find_by_state_id("13").discipline_incidents.last }
			...
			context 'description text has non UTF-8 byte sequence' do
				let(:incident_with_non_utf8_description) { Student.find_by_state_id("12").discipline_incidents.last }
				it 'fights back' do
			...
			context 'date missing' do
				let(:student_with_incident_without_date) { Student.find_by_state_id("14") }
