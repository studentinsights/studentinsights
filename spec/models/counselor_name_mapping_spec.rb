...
RSpec.describe CounselorNameMapping do
  ...
  describe '.has_mapping?' do
    it 'finds existing TestPals mapping' do
      expect(CounselorNameMapping.has_mapping?(pals.shs_sofia_counselor.id, 'sofia')).to eq true
    end
    ...
    it 'considers text when looking for mapping' do
      expect(CounselorNameMapping.has_mapping?(pals.shs_sofia_counselor.id, 'sophie')).to eq false
    end
  end
end
