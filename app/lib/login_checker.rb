...
class LoginChecker
  ...
  def alert_about(flags)
    ...
    # alert developer
    with_isolation do
      Rollbar.error('LoginChecker#alert_if_suspicious', {
        flags: flags,
  ...
  def send_email_to_user!(educator_email, warning_id)
    ...
    post_data = Net::HTTP.post_form(URI.parse(mailgun_url), {
      ...
      :subject => "Security alert for #{PerDistrict.new.canonical_domain}",
  ...
  def user_facing_email_text(warning_id)
    [
      "Your #{PerDistrict.new.canonical_domain} account was just signed into.  You're getting this email to make sure that it was you.",
      ...
      "",
      "Security alert sent from #{PerDistrict.new.canonical_domain} at #{@time_now}."
    ].join("\n\n")
  end
end
