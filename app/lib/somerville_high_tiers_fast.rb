...
class SomervilleHighTiersFast
  FAILING_GRADE = 65

  class Tier < Struct.new(:level, :reason, :data)
  end

  def tier_all(options = {})
    ...
    @students = Student.all.where(school_id: school_ids)
  ...
  def course_failures(student)
    student_assignments = @student_section_assignments.where(student_id: student.id)
    student_assignments.select do |assignment|
  ...
  def recent_discipline_actions(student)
    discipline_events_count = @discipline_incidents
      .where(student_id: student.id)
      .where('occurred_at > ?', @time_now - @discipline_time_interval)
      .size
