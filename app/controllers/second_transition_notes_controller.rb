class SecondTransitionNotesController < ApplicationController
  ...
  def create_json
    ...
    render json: {
      second_transition_note_id: second_transition_note.id
    }
  end
  ...
  def update_json
    ...
    second_transition_note.update!({
      form_json: safe_params[:form_json],
      is_starred: safe_params[:is_starred],
  ...
  def verify_authorized_or_raise!(student_id, second_transition_note_id)
    ...
    second_transition_note = student.second_transition_notes.find_where(id: second_transition_note_id)
