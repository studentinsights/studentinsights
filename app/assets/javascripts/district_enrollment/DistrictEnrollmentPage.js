...
function computeStudentCountForSchool(school, enrollments) {
  return enrollments
    .filter(enrollment => enrollment.school.id === school.id)
...
export default class DistrictEnrollmentPage extends React.Component {
  constructor(props) {
    super(props);
    this.fetchEnrollmentData = this.fetchEnrollmentData.bind(this);
    this.renderEnrollment = this.renderEnrollment.bind(this);
  }

  fetchEnrollmentData() {
    const url = `/api/district/enrollment_json`;
    return apiFetchJson(url);
  }

  render() {
    return (
      <div className="DistrictEnrollmentPage">
        <ExperimentalBanner />
        <GenericLoader
          promiseFn={this.fetchEnrollmentData}
