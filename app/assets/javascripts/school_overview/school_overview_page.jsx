...
(function (root) {
  ...
  window.shared.SchoolOverviewPage = React.createClass({
    ...
    filterWithOr(filterGroups) {
      ...
      return filterGroups.map(filterGroup => filterGroup.reduce((filteredStudents, filter) => _.uniq(filteredStudents.concat(allStudents.filter(filter.filterFn))), []));
    },

    filterWithAnd(studentArrays) {
    ...
    filteredStudents() {
      ...
      if (maxFiltersPerCategory === 1) {
        return this.state.filters.reduce((filteredStudents, filter) => filteredStudents.filter(filter.filterFn), this.props.allStudents);
      }
