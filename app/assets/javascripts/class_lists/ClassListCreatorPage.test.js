...
it('integration test for state changes, server requests and autosave', done => {
  ...
  // Wait for school and grade levels to load.
  Promise.all([
    new Promise(resolve => setTimeout(resolve, 2000)),
    new Promise(resolve => {
      console.log(wrapper.state());
      expect(wrapper.state().hasFetchedGradeLevels).toEqual(true);
      expect(wrapper.state().schoolId).toEqual(null);
      expect(wrapper.state().gradeLevelNextYear).toEqual(null);
      // expect(wrapper.state().schools.length).toBeGreaterThan(0);
      // expect(wrapper.state().gradeLevelsNextYear.length).toBeGreaterThan(0);
      resolve();
    }),
  ]).then(() => done());
